...
// --- Clase Particle ---
class Particle {
    constructor(x, y, color, startFromBottom = false) {
        this.velX = 0;
        this.velY = 0;
        this.friction = 0.93;
        this.ease = 0.04;
    ...
    draw(ctx, time) {
        ...
        ctx.beginPath();
        const distFromMouse = Math.sqrt(Math.pow((mouse.x - this.x), 2) + Math.pow((mouse.y - this.y), 2));
        const maxBlur = 30;
...
let particles = [];
let lastText = ""; // Guarda el último texto para poder recrearlo al redimensionar
// --- MODIFICACIONES CRÍTICAS EN createTextParticles para Reubicación y Densidad ---
function createTextParticles(text, isResizing = false) {
    ...
    const newParticlesArray = []; // Array temporal para las partículas válidas
